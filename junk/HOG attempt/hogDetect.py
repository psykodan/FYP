...
def main():
	...
	svm = str(input("Enter path to Haar cascade classifier xml file: ") or '/home/daniel/Documents/FYP/FYP/hog/svm.xml')
	assert os.path.exists(haar), "Error: File does not exist at: , "+str(haar)
	#cascade = cv2.CascadeClassifier(svm)

	#threshold = float(input("Enter a threshold value of Haar cascade classification confidence: ") or 0)
	
	haarDetection(DIR, svm)



def haarDetection(inputFile, cascade):
	hog = cv2.HOGDescriptor()
	hog.setSVMDetector(cascade)
