...
def main():
	...
	for hSetNum in range(len(holdoutSets)):
		for i in range(51):
			threshold = i * 0.1
			haarDetectionImageStream(holdoutSets[hSetNum], cascade, threshold)
			processResults(objectLocations[hSetNum], threshold, holdoutSets[hSetNum])
			print(i)


def processResults(info, threshold, dirIn):
	...
	rR.write(str(threshold) +","+ str(p) +","+ str(n) +","+ str(tp) +","+ str(tn) +","+ str(fp) +","+ str(fn) +","+ str(accuracy) +","+ str(precision) +","+ str(sensitivity) +","+ str(specificity) +"\n" )
	rR.close()
